...
from pbasex import rBFs

try:
	import dill
except:
	def cpu_count():
		print('Install dill if you would like to use multiple cores to generate pBASEX basis functions.')
		return 1
else:
...
def leg(l,x):
	...
	if l==0:
		...
		#Calculate the Legendre polynomial (only works up to 28, after that the
		...
		return np.divide(legendre_poly(x), np.math.factorial(l))
	else:
